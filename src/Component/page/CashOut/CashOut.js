...
import { fetchAgentBankAccounts, sendOtpToAgent, walletCashOutFromBank } from "../../../services/agent/action.js";
...
const CashOut = () => {
    ...
    const walletCashOutLoading = useSelector(state => state.agentReducer.agentWalletCashOut.loading);
    const walletCashOutSuccess = useSelector(state => state.agentReducer.agentWalletCashOut.success);
    ...
    const stepFourValidated = () => {
        return !(validator.isEmpty(otp) || otp.length !== 6 || walletCashOutLoading);
    };
