...
const UpgradeToAgentBanker = () => {
  ...
  useEffect(() => {
    ...
    else setShowAgentBankerDetails(false);
          ...
          <div className="containerBiaN_f_row">
            <div className="containerBiaN_f_col">
              <div style={{ display: "flex" }}>
                <p style={{ marginRight: "16px", color: "gray" }}>Bank Name</p>
                <p style={{ fontWeight: "bold" }}>
                  {agentBankerUpgradeData.customerBankName}
                </p>
              </div>
              <div style={{ display: "flex" }}>
                <p style={{ marginRight: "16px", color: "gray" }}>Bank Phone</p>
                <p style={{ fontWeight: "bold" }}>
                  {agentBankerUpgradeData.customerBankPhone.replace(
                    /\b[\dX][-. \dX]+(\d{4})\b/g,
              ...
              <div style={{ display: "flex" }}>
                ...
                >{`${agentBankerUpgradeData.customerBankEmail}`}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ marginTop: "40px" }}>
            <div className="confirm_p_w button-container rspacing">
              <button
                ...
                style={{
                  opacity: customerID === "" ? "0.5" : "1",
                  cursor: customerID === "" ? "no-drop" : "pointer",
                }}
                    ...
                    >
                      <div style={{ margin: "16px 0px" }}>
                        {!showAgentBankerDetails
                          ? bankCustomerIDForm()
                          : showAgentBankerDetails &&
                            agentBankerUpgradeStatus &&
                            agentBankerUpgradeData && (
                              <>{transactionDetails()}</>
                            )}
                      </div>
                    </div>
